...
for c1 = 1:size(subject_idx,2)
    ...
    subplot(2,2,1)
    v1 = squeeze(sample_v(1,:,:));
    plot(v1, 'color', .5*ones(1,3));
    ylim([0 4]);
    xticklabels({})
    title('Default Bayesian Opt')
    set(gca, 'FontSize', 16);
    
    subplot(2,2,2)
    v1 = squeeze(sample_v(2,:,:));
    plot(v1, 'color', .5*ones(1,3));
    ylim([0 4]);
    title('Model Hyperprior')
    xticklabels({})
    yticklabels({})
    ...
    subplot(2,2,3)
    v1 = squeeze(sample_v(3,:,:));
    plot(v1, 'color', .5*ones(1,3));
    ylim([0 4]);
    title('Safety Constraints')
    ylabel('Sampled Voltage');
    xlabel('Sample');
    set(gca, 'FontSize', 16);
    
    subplot(2,2,4)
    v1 = squeeze(sample_v(4,:,:));
    plot(v1, 'color', .5*ones(1,3));
    ylim([0 4]);
    title('Hyperprior + Safe-Opt')
    yticklabels({})
    set(gca, 'FontSize', 16);
